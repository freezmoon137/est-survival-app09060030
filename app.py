...
def load_est_model():
            ...
            try:
                ...
                if isinstance(loaded_data, dict):
                    ...
                    if 'model' in loaded_data:
                        ...
                        logger.warning(f"字典中未找到'model'键，尝试直接使用加载的数据")
    ...
    # 如果没有找到预训练模型，创建一个简单的默认模型
    logger.warning("未找到预训练模型，创建默认模型用于演示")
    try:
        # 创建一个简单的默认模型（使用自定义的ExtraSurvivalTrees类）
        est_model = ExtraSurvivalTrees(n_estimators=50, random_state=42)
        model_loaded = True
...
def create_default_model():
    """创建默认EST模型"""
    logger.warning("scikit-survival不可用，无法创建默认模型")
    return None
